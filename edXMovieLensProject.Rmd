...
The prediction of a given user's rating of a given movie might be simply estimated by taking the average of all observed movie ratings. Expanding on this, we take the average of each movie's rating across all users, and each users rating across all movies. This model takes into account the effect of the movie, as well as the user. Using this model a RMSE of `r user_movie_RMSE` is acheived.

The first observation is that some movies are rated more highly than others, as might be expected.explains the process and techniques used, including data cleaning, data exploration and visualization, insights gained, and your modeling approach
